    ...
    public class ConversationRepository : IConversationRepository{
        ...
        public void AddMessage(Conversation conversation, Message mes){
            ...
            string query = $"INSERT INTO messages(Conversation_Id, Sender_Id, Message_Type, Message, Attachment_url, Create_at) " +
                $"VALUES ({conversation.Id}, {mes.Sender.Id}, '{mes.TypeMessage.ToString().ToLower()}', N'{mes.Content}', '{mes.FileAttachUrl}', now())";
            dataProvider.ExcuteQuery(query);
        }
        public void AddConversation(User user1, User user2){
            string query = $"INSERT INTO conversation(Title, Creator_Id, Create_at, Update_at, Delete_at) VALUES (N'{user1.LastName + " " + user1.FirstName}', {user1.Id}, now(), now(), NULL)";
            dataProvider.ExcuteQuery(query);
        ...
        public void AddConversation(string nameConversation, User creator, User[] listParticipants){
            ...
            string query = $"INSERT INTO conversation(Title, Creator_Id, Create_at, Update_at, Delete_at) VALUES (N'{nameConversation}', {creator.Id}, now(), now(), NULL)";
            dataProvider.ExcuteQuery(query);
            int idConversation = GetNewestConversationId();
