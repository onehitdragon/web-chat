            ...
            if(user.Id != 0){
                if(GetUser(user) == null){
                    string query = $"INSERT INTO Users(id , AvatarUrl, FirstName, LastName, BirthDay, Gender, Phone, TimeCreated)"
                        + $"VALUES({user.Id},'{user.AvatarUrl}',N'{user.FirstName}',N'{user.LastName}','{user.BirthDay.ToString("yyyy/MM/dd")}', {Convert.ToInt32(user.Gender)} , '{user.Phone}', now())";
                ...
                string query = $"INSERT INTO Users(AvatarUrl, FirstName, LastName, BirthDay, Gender, Phone, TimeCreated)"
                    + $"VALUES('{user.AvatarUrl}',N'{user.FirstName}',N'{user.LastName}','{user.BirthDay.ToString("yyyy/MM/dd")}', {Convert.ToInt32(user.Gender)} , '{user.Phone}', now())";
                dataProvider.ExcuteQuery(query);
            }   
        }
